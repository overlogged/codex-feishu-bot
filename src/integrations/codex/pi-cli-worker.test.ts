...
test("buildPiCliArgs includes the persistent group goal for pi turns", () => {
  const args = buildPiCliArgs(
    {
      PI_CLI_PROVIDER: "openrouter",
      PI_CLI_MODEL: "deepseek-v4-pro",
    ...
    },
    {
      ...
      session: {
        chatId: "oc_group_1",
        threadId: "/home/overlogged/.pi/agent/sessions/codex-feishu-bot/oc_group_1/session.jsonl",
        cli: "pi",
        workspaceId: "/home/overlogged/QuantDev",
        executionMode: "host",
        goal: "每次改代码前先看测试",
  ...
  assert.match(prompt, /Persistent chat goal/);
  assert.match(prompt, /每次改代码前先看测试/);
});

test("buildPiCliArgs explicitly clears earlier persistent goals for pi turns", () => {
  const args = buildPiCliArgs(
    {
      PI_CLI_PROVIDER: "openrouter",
      PI_CLI_MODEL: "deepseek-v4-pro",
  ...
  const prompt = args.at(-1) ?? "";
  assert.match(prompt, /No persistent chat goal is currently set/);
  assert.match(prompt, /Ignore any previous persistent chat goal sections/);
});
